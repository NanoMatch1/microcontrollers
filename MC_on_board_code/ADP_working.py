...
def process_command(command, reset_pin, reader_sm):
    if command[0].strip() == 'acquire':
        try:
            acq_time = float(command[1].strip())
        ...
        total_counts = acquire_signal(duration, reset_pin, reader_sm)
        return total_counts
...
def wait_for_command():
    while True:
        if select.select([sys.stdin], [], [], 0)[0]:
            ...
            ch = ch.strip()
            command = ch.split(' ')
            print('APD Pico received {}'.format(ch))
            return command
        else:
            time.sleep(0.01)
            continue
...
# Setup I2C
    ...
    while True:
        ...
        result = process_command(command, reset_pin, reader_sm)
        print(result)
    #print(result)
